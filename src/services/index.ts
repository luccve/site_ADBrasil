...
class RequestCoordsService {
        ...
        try {
            ...
            if (!data_adbrasil_malha.response.ok) {
                ...
                switch (data_adbrasil_malha.data.features[0].properties.sigla_uf) {
                    ...
                    // case 'PB':
                    //     const data_pb = await this.fetch_region(this.url_geinfo_pb_ad_clip_wfs(lat, lng));
                    //     console.log(data_pb.data)
                    //     return {
                    //         resposta: 200,
                    //         geojson: {
                    ...
                    //         ct_c4: data_pti_geoinfo.data.features[0].properties.ct_c4,
                    //         ct_c5: data_pti_geoinfo.data.features[0].properties.ct_c5,
                    //         região: data_adbrasil_malha.data.features[0].properties.sigla_uf,
                    //         classe_terra: data_pti_geoinfo.data.features[0].properties.pti_um,

                    //     }
                        
                    // case 'SP':
                    //     const data_sp = await this.fetch_region(this.url_geinfo_sp_ad_clip_wfs(lat,lng));
                    //     console.log(data_sp.data);
                    //     return {
                    //         resposta: 200,
                    //         geojson: {
                    ...
                    default:
                        ...
                        const data_geoinfo_adbrasil_wfs = await this.fetch_region(this.url_geinfo_adbrasil_wfs(lat, lng));
    ...
    // private static url_geinfo_sp_ad_clip_wfs(lat: number, lng: number) {
    //     const cqlFilter = `INTERSECTS(geom,POINT(${lng} ${lat}))`;
    ...
    // private static url_geinfo_pb_ad_clip_wfs(lat: number, lng: number) {
    //     const cqlFilter = `INTERSECTS(geom,POINT(${lng} ${lat}))`;
